...
Next,
footnote:[Are you unable to move on because you're wondering what those
...
In the Red/Green/Refactor dance, we've got to green, so we should see what
...
`test_home_page_only_saves_items_when_necessary` test too!
...
And we re-run our FTs to make sure everything still works, or works at least as
...
So just to reassure ourselves that things have worked, we re-run the FT.  Sure
...
`printf` function from C, that `%d` may be a little confusing?
<<dip,Dive into Python>> has a
http://www.diveintopython.net/native_data_types/formatting_strings.html[good
overview], if you want to go look them up quickly.  We'll see an alternative
string substitution syntax later in the book too.
...
of thumb like "YAGNI" and "3 strikes then refactor". But, most importantly,
